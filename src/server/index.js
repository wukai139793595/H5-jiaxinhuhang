...
import {getSessionStorage, setSessionStorage, removeSessionStorage, getUrlKey,removeLocalStorage} from '@/common/js/mixin';
...
Axios.interceptors.request.use(config => {
  ...
  if (sessionStorage.getItem('loginData')) {
    config.headers.Authorization = JSON.parse(sessionStorage.getItem('loginData')).token;
  }
...
Axios.interceptors.response.use( res => {
	...
	if (res.status == 200) {
	    console.log('res:',res)
        if(res.data.code == '5002' || res.data.code == '5003') {// 5002token为空，5003token超时
